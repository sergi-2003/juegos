...
class MenuManager {
    ...
    async loadRealLeaderboardData() {
        try {
            ...
            // Cambia la URL para usar el host local
            const apiBase = window.location.origin.includes('localhost') ? window.location.origin : 'http://localhost:3000';
            const response = await fetch(`${apiBase}/api/scores/leaderboard?limit=10`);
